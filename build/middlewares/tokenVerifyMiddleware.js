...
const tokenVerifyMiddleware = () => {
    return (req, res, next) => {
        try {
            ...
            if (!decoded) {
                throw new customError_1.CustomError(`Unauthorized: Empty or Invalid JWT token. Please sign in, or sign up again.`, 401);
            }
            if (req.session) {
            ...
            if (err instanceof customError_1.CustomError)
                next(err);
            next(new customError_1.CustomError(`Unauthorized: Empty or Invalid JWT token. ${err}`, 401));
        }
    };
};
